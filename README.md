...
Run the unit tests with `npm test`, the RBAC matrix with `node scripts/dev/permissions-audit.mjs`, and the responsive check with `node scripts/dev/responsive-audit.mjs` (both need the dev server running).
...
- `SERVO_ENCRYPTION_KEY` — encrypts stored secrets (API keys, tokens, SMTP URLs) at rest with AES-256-GCM. Existing plaintext rows migrate with `node scripts/ops/encrypt-secrets.cjs`.
...
- **Locked out by a bad SSO config?** `node scripts/ops/reset-sso.cjs` clears the OIDC tenant (keeping admin emails/domains) and drops Servo back to demo mode so you can fix it from `/integrations`.
