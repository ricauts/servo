...
// The ops sandbox is a Postgres database as of db-05 — same adapter the app
// uses (src/lib/opsdb.ts), with ? placeholders converted to $n at this one
// boundary because the seed's statements read better with positional ?s.
import { opsExecute, opsDisconnect } from "../src/lib/opsdb";

const db = new PrismaClient();

function dollar(sql: string): string {
  let i = 0;
  return sql.replace(/\?/g, () => `$${++i}`);
}

async function opsRun(sql: string, params: unknown[] = []): Promise<void> {
  await opsExecute(dollar(sql), params);
}
...
async function main() {
  ...
  // The knowledge base and its sources reference users (owner, granter,
  // creator) and would otherwise refuse the wipe with an FK error on a desk
  // that indexed anything — the cascades below take chunks, facts, edges and
  // grants with their documents.
  await db.catalogRun.deleteMany();
  await db.catalogEntry.deleteMany();
  await db.document.deleteMany();
  await db.kbGrant.deleteMany();
  ...
  for (const table of [
    ...
    await opsRun(`DROP TABLE IF EXISTS ${table};`);
  }
  ...
  await opsRun(`
    CREATE TABLE employees (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT NOT NULL, email TEXT NOT NULL,
  ...
  for (const d of devices) {
    await opsRun(`INSERT INTO devices (asset_tag, model, type, assigned_to, status, os, purchased_at, warranty_until)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, d);
  }
  ...
  for (const e of employees) {
    await opsRun(`INSERT INTO employees (id, name, email, department, title) VALUES (?, ?, ?, ?, ?)`, e);
    await opsRun(`INSERT INTO employees_backup (id, name, email, department, title) VALUES (?, ?, ?, ?, ?)`, e);
  }
  ...
  for (const l of licenses) {
    await opsRun(`INSERT INTO software_licenses (id, product, seats, seats_used, renewal_date, owner_email)
       VALUES (?, ?, ?, ?, ?, ?)`, l);
  }

  await opsRun(`INSERT INTO campaign_tracking (campaign, channel, spend_usd, leads, created_at)
  ...
  });
  {
    ...
    const ddl =
      "CREATE TABLE campaign_tracking (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, campaign TEXT NOT NULL, channel TEXT NOT NULL, spend_usd REAL NOT NULL DEFAULT 0, leads INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL);";
    const steps = [
      ...
      { type: "TOOL_CALL", toolName: "query_ops_database", riskLevel: "LOW", content: JSON.stringify({ sql: "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name" }) },
      { type: "TOOL_RESULT", toolName: "query_ops_database", content: JSON.stringify([{ name: "devices" }, { name: "employees" }, { name: "employees_backup" }, { name: "software_licenses" }]) },
