...
- **SQL reads run as a read-only role.** `xds-04` depends on `db-05`, which is the item that establishes the pattern (`ALTER ROLE … SET default_transaction_read_only = on`, plus `BEGIN … SET TRANSACTION READ ONLY` around every statement). **Do not cite `src/lib/opsdb.ts:19/41` as the precedent for a Postgres read-only transaction — today that file is a SQLite client pinned with `PRAGMA query_only`.** What it does establish, and what `xds-04` reuses, is the *shape*: a second `PrismaClient` bound by `datasourceUrl` with `$queryRawUnsafe` over Servo-composed SQL, pinned read-only at the driver rather than by keyword filtering. **No new SQL driver** — `pg` and `postgres.js` buy nothing here. Be honest about the asymmetry: when MSSQL eventually lands there is no `SET TRANSACTION READ ONLY`, and the guarantee there rests on `db_datareader` plus Servo composing every statement. That is weaker, and it is one reason MSSQL is not v1.
