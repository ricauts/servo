...
| `docs/history/CONTRACT.md:19, 26` | "Prisma 6 + SQLite", "SQLite has no enums" | Postgres; the string-union rule survives with a new reason |
...
- The canned SQL in the deterministic mock provider (`src/lib/ai/mock.ts:250`), the fixture step in `prisma/seed-demo.ts:584`, the example in `docs/history/CONTRACT.md:171`, `agents/analytics-agent.md:14` and `skills/production-database-change/SKILL.md:18` all move off `sqlite_master` to `SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name`.
