...
51. **2026-08-30 — `safeFetch` replays the `Authorization` header across a cross-origin redirect, which is STRICTLY WEAKER than the platform default, and `cnp-02` is the change that routes a newly-stored credential through it.** Found by `cnp-02`'s adversarial verification, outside that item's criteria. `src/lib/egress.ts:288-307` follows redirects itself (`redirect: "manual"`, re-checking each hop) and re-issues **the same `init`** — headers included — at every hop. Measured, not reasoned: a fixture at host R issuing a 307 to host S delivered `Authorization: Bearer <token>` to S; the control experiment with native `fetch` and `redirect: "follow"` across the identical cross-origin hop delivered `Authorization: null`, because Node/undici strips it per the fetch spec. Since the default allowlist is empty and `matchAllowlist` reads empty as "every public host allowed" (`egress.ts:105-106`), any 3xx from a configured MCP endpoint — an expired vendor domain, a CDN misconfiguration, a vendor-side open redirect — hands that vendor's bearer token to a third party, up to five different hosts per sync. **The loop did NOT fix it.** `src/lib/egress.ts` is a permanent Tier-C surface (§0.6 rule 2) that `cnp-02` does not name, and §0.2 step 8 makes scope discipline absolute; the fix is one place — drop `Authorization` (and any header carrying a `{secret}` substitution) when a hop changes origin, at `egress.ts:306`. Note this is **pre-existing**: admin-defined HTTP custom tools with a `{secret}` header (`src/lib/ai/custom-tools.ts:53-59`) have had the same exposure since they shipped, so this is not a `cnp-02` regression — it is a defect `cnp-02` made worth naming. What `cnp-02` DID do at its own layer: the remote server's error text is scrubbed of the token before it reaches an admin's screen or a log (`redactSecret`, `src/lib/mcp-client.ts`), because a fixture that echoed the `Authorization` header back in a 500 body got it rendered verbatim on the connection panel before that fix.
    *Related, lower severity, and deliberately left consistent rather than "fixed" one-sidedly:* `McpServer.headers` is returned raw by the API, exactly as `CustomTool.headers` already is. An admin who pastes a literal token into `headers` instead of using `{secret}` gets it stored unencrypted and echoed back. Only the `secret` column is sealed. Changing one of the two and not the other would be worse than either; deciding both is yours.

52. **2026-08-30 — nothing caps the SIZE of a `tools/list` response body, and the cap that would fix it belongs inside `src/lib/egress.ts`.** Also from `cnp-02`'s verification. `cnp-02` bounds what a remote server may PERSIST — at most 200 tools per sync, tool names shaped `^[A-Za-z0-9_.-]{1,64}$` or skipped, descriptions capped at 1000 characters and flattened, an input schema over 20 kB dropped rather than stored — and a test drives a 400-tool fixture with 40 kB schemas to prove it. What it cannot bound from its own layer is the transfer: the SDK buffers the whole JSON-RPC body in memory before `listTools()` resolves, so a multi-gigabyte response is a memory spike before any of Servo's caps are reached. A measured 40 MB response produced a 275 MB heap delta. The bound has to be a streaming byte counter in the `safeFetch` wrapper — the pattern `dcl-03` already uses for the Docling sidecar — and that is `src/lib/egress.ts`, the same Tier-C surface as question 51. Suggested resolution: settle 51 and 52 in one owner-approved `egress.ts` change (strip credentials on cross-origin redirect; add an optional `maxBytes` to `safeFetch`), which would also give the custom-tool executor a cap it does not have today.
...
| 2026-08-30 | cnp-02 | **Servo becomes an MCP client — and every tool it finds arrives in quarantine.** ADOPT-FIRST verdict: **`@modelcontextprotocol/sdk` ADOPTED — MIT, verified by reading `node_modules/@modelcontextprotocol/sdk/LICENSE` ("Copyright (c) 2024 Anthropic, PBC") on 2026-08-30, not from memory**, moved from devDependencies to `dependencies` (1.30.0) and recorded in `THIRD_PARTY.md` with the alternative it rejects (a hand-rolled JSON-RPC/SSE client, which is why this item has no multi-event-SSE degradation criterion). `src/lib/mcp-client.ts` constructs the SDK `Client` + `StreamableHTTPClientTransport` and builds no JSON-RPC envelope and parses no SSE itself; a test greps the file for both. `model McpServer {slug @unique, transport String default "http", url, headers, secret, enabled default false, toolsJson, lastSyncAt}` lands in `prisma/migrations/0009_mcp_server` (0006 was already taken by `skill_source_ticket`; the item's `files` hint names the number, not the contract) — CREATE TABLE + CREATE UNIQUE INDEX only, which `scripts/migration-guard.mjs` classifies additive, and `prisma migrate diff` reports zero drift against `schema.prisma`. `McpServer.secret` joins the `$extends` seal hook in `src/lib/db.ts` (create/update/upsert) and is opened at exactly one site — `buildHeaders()`, at `{secret}` substitution time; the sealing test drives the REAL extended client and reads the column back with raw SQL to see the sealed-value prefix and ciphertext, not the token, and every API payload carries `secretSet` with no `secret` key. Admin CRUD at `/api/mcp-servers` (+ `?action=sync`) sits behind `settings.manage` on all five handlers, refuses a non-http or credentialed URL and any transport but `http`, and creates every connection `enabled: false`; the list renders inside `/integrations` as its own section, so no page and therefore no `NavEntry` is added. `syncMcpServerTools()` lists through the SDK **over `safeFetch`** — the initialize POST, the standalone GET and tools/list all pass `checkEgress`, so a loopback fixture is unreachable until the test names `127.0.0.1:<port>` literally in the outbound allowlist, and an unlisted host mints nothing and does not stamp `lastSyncAt`. Each tool is snapshotted with `sha256(name + description + canonical(inputSchema))` and gets a `ToolPolicy` row named `mcp__<slug>__<tool>` carrying the Ruling-6 triple `{enabled: false, requiresApproval: true, riskLevel: "HIGH"}`; a risk level a server declares in `_meta` is recorded on the snapshot and **ignored** (`annotations` is a closed schema the SDK strips, so a declaration parked there never even arrives). Sync is create-only for a row this server has already synced against, with the tighten-only exception in two shapes that say one thing: **a row may only stay loose while Servo can show a human reviewed THIS tool, as it is now, on THIS server** — DRIFT (hash moved under an admin who enabled it) and ADOPTION (the row exists with no entry in this server's own snapshot). Adoption is the hole independent verification found and this closes: DELETE keeps policy rows on purpose, so without it a deleted-and-recreated slug pointed at a NEW host silently inherited the approval granted to the server it replaced — reproduced end to end, then fixed and pinned by a test. Re-pointing the connection clears the snapshot for the same reason, so a URL swap lands in the same case — and **`headers` or `secret` count as re-pointing too**, because `buildHeaders()` substitutes the token into the request and on a multi-tenant or header-routed endpoint those are what select the tenant and the backend behind an unchanged address; a corrupt `toolsJson` lands there as well. The snapshot write is **conditional on the identity this sync actually talked to** (`updateMany` keyed on id + url + headers + secret): listing is a network round trip of up to 15 s, and an unconditional write let an in-flight sync silently restore the baseline a PATCH had just cleared — a race a verifier reproduced end to end. Zero rows updated now means the connection moved underneath the sync: the tightening writes stand, the snapshot does not, and the next sync therefore sees adoption. Both shapes write the frozen triple, so neither can loosen. **Two independent adversarial verification rounds ran against this diff — the first found four real defects, the second found four more in the fixes for the first, and every one of the eight is either closed above or filed below.** The never-loosens claim is proved as a **36-case matrix — all 12 enabled × requiresApproval × riskLevel combinations × {no-change, drift, adoption}** — asserting per field that `enabled` only went true→false, `requiresApproval` only false→true, `riskLevel` never fell, and an admin's `description` survives an unchanged sync. Nothing deletes a policy for a tool that vanished. **A `tools/list` response is treated as attacker-shaped data**, because every part of it lands in a table: at most 200 tools per sync, a name outside `^[A-Za-z0-9_.-]{1,64}$` SKIPPED rather than mangled (mangling is how a collision is minted — a 5041-character name became a `ToolPolicy` PRIMARY KEY before this), duplicates de-duplicated and `skipDuplicates` on the insert so a racing sync cannot throw, a schema over 20 kB dropped, and **both the description and the input schema hashed BEFORE bounding, stored after** — hashing what was stored let a server pad past a cap and then rewrite the tool's meaning beyond it forever without ever reading as drift, and with the schema that was worse, since every oversized schema bounds to the same placeholder and so hashed identically (a verifier swapped `harmless_text` for `shell_command_to_run` past the cap and nothing fired). The description is also flattened to one line, because the engine interpolates an ENABLED row's description into the resolver system prompt — and the class is enumerated rather than left to `\s`, since the separators that matter are the ones `\s` misses: NEL, the zero-width and bidi format characters, and the BOM. A drifted tool's **description is refreshed** on the row even while it stays quarantined: the stored text is what an admin reads before deciding, the baseline advances on every sync, and a description frozen at mint time is a bait-and-switch no later drift check can catch. Refreshing text is not a policy write; an unchanged sync still never touches it, which the matrix asserts. `listRemoteTools` never throws: `getEgressConfig()` and the `open()` of the sealed secret moved inside its try, and the remote server's error text is scrubbed of the bearer token before an admin or a log sees it. Both handshake and `tools/list` are bounded at 15 s, proved by a fixture that accepts the connection and then says nothing. `POST /api/tools` now refuses any custom tool named `mcp__*`, reserving the namespace. `tests/tool-policy-invariant.test.ts` allowlists the new policy-creation site and pins the file's `MCP_QUARANTINE` constant to `policy-guard.mjs`'s `QUARANTINE_TRIPLE` so the two cannot drift. **Nothing here makes an MCP tool executable** — `getToolRegistry()` is untouched and every minted row is disabled; execution is `cnp-03`. `npm run typecheck` and `npm test` green offline (73 files, 1026 tests, +45), `next build` clean, hex/claims/hygiene/spec/policy guards all pass. Tier C by rule twice over — a new runtime dependency and a schema migration — so: PR, status `review`, not merged. **Two defects the verification found and this tick deliberately did NOT fix are filed as owner questions 51 and 52**: `safeFetch` replays the `Authorization` header across a cross-origin redirect (pre-existing, and it already affects custom HTTP tools), and no byte cap bounds a response body. Both fixes live in `src/lib/egress.ts`, a permanent Tier-C surface this item does not name. | PR: feat/cnp-02 (awaiting owner review) |
