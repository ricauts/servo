...
| 2026-08-29 | db-05 (owner-directed twin-PR resolution) | The concurrent-session collision on db-05 is resolved the way question 42 rules: the implementation on `main` (merge 059d0af) stays, PR #14 — the other full implementation, CONFLICTING with it file-for-file in opsdb/bootstrap/tests — is closed superseded and NOT re-litigated, and this branch lands only the deltas `main` genuinely lacked, lifted from PR #14 where they applied verbatim: Dockerfile drops the stale `OPS_DATABASE_URL="file:/data/ops.db"` default (the sandbox is Postgres; /api/setup and the ops tools name the variable when it is unset), .env.example activates the sandbox URLs with the passwords scripts/postgres-init.sql actually sets (`servo_ops_rw`/`servo_ops_ro` — the committed example said `servo_ops`, which matches no role), README drops "enforced at the driver" for the role-plus-read-only-transaction wording, ROADMAP and ARCHITECTURE drop the last pre-db-05 sandbox claims, POSITIONING rewrites the compose ledger row and retires the spent `until: db-05` sqlite exemption (docs/CONTRACT.md and docs/ARCHITECTURE.md are sqlite-free, so it guarded nothing). PR #15 (the same kb-pdf null fix `main` already carries) is closed redundant; its stronger not-null assertion is adopted here with credit to its author-session (30e6186). The duplicated `date:` line the 059d0af conflict resolution left in this item's block is removed. | merge `fix/db-05-deploy-claims` |
