...
44. **`react-is` is declared to satisfy a peer dependency, not to be imported — should the declaration stay?** Found while `hyg-04` was writing an owner onto every baseline row and this one had none. `scripts/repo-refs.mjs` reports it *declared, never imported*, which is true: nothing in the tree imports it. It is not stray, though — `package-lock.json` records `node_modules/recharts` requiring it under `peerDependencies`, and it is the only root dependency in that shape. npm 7+ installs peer dependencies automatically, so the explicit line may be redundant; it is also the only thing that pins the version rather than letting the resolver pick one. `hyg-05` is scoped to `gifenc` and to nothing else, so no item currently owns this line either way.
    *Assumption: it stays, with a baseline row naming the lockfile entry as its evidence, and no item removes it. A dependency line is only removable under the deletion rule with evidence, and "the resolver would probably install it anyway" is not evidence. If you would rather drop it, that is a one-line change plus a `npm i --package-lock-only`, and it wants its own item because a removed runtime dependency is Tier C.*
...
| 2026-08-29 | hyg-04 | `node scripts/repo-refs.mjs --check` becomes the gate hyg-01 said it would add, wired as `npm run hygiene:check` and as its own CI step beside the claims audit. It fails on exactly three regressions — a file that became unreferenced with no baseline row, a declared dependency nothing imports, an imported module in no manifest — plus a fourth, an unreadable or unowned baseline, because a row with no reason and no owner excuses nothing. `tests/fixtures/repo-refs-baseline.json` is a KEEP-LIST: 17 rows (12 files, 5 packages), each with a one-line reason and an owner resolved against `spec.md` by a test, so `hyg-05`, `hyg-08`, `hyg-09`, `cnp-02`, q31 and q43 each carry the ones they will retire. It removes nothing: `gifenc` is still declared, and a test asserts it. TWO TRAPS, both hit and both now encoded. (1) The baseline is added to `NON_REFERENCING_SOURCES`: `.json` is a mention extension, so a baseline counted as a referencing source makes every path it lists read as `referenced`, the findings vanish and the gate passes because it stopped looking — the baseline would launder its own rows. (2) A quote or backtick inside a REGEX literal defeats `maskCode()`, which tracks strings but not regex context; the mask inverts, the prose in `RESOLVER_RULES` parses as code, its "non-literal `import()`" sentence reads as a real dynamic import and the WHOLE repository is marked INDETERMINATE — 73 files silently became reachable. Caught by diffing the report against the previous commit, which is now how this item proves it changed no verdict: the report is byte-identical to `main`'s except the file count (484 -> 486) and line-number drift. The package rows are keyed `packages`, not `dependencies`: `landing-tier.mjs` tracks manifest dependency blocks by header line across the whole staged diff, so the natural key made this diff classify **C** on a package.json change that only adds an npm script. Renaming the key fixed the classifier's input rather than the classifier — it now says **A**, matching the hint. `docs/MEDIA-GUIDE.md` does not exist yet and its absence is not a failure, as the item requires; `parseMediaAllowlist` reads it when `hyg-09` writes it, and the allowlist excuses only the no-manifest shape, never a declared-and-unused package. Adopt-first: **nothing adopted, ideas only.** `knip` (ISC, Copyright 2022-2026 Lars Kappert) and `depcheck` (MIT, Copyright 2015 Djordje Lukic, Junle Li) were both read from their upstream LICENSE files and both clear the licence gate, but each would REPLACE the scanner six backlog items name by path, add a dependency to a tool hyg-01 built on Node builtins, and neither produces a keep-list carrying an owning item id. knip's baseline-with-reasons shape is the idea borrowed. `npm run typecheck` clean; **505 tests green across 25 files** (484 before, 21 new); `hygiene:check`, `claims:audit`, `spec-lint`, `loop-guard` and `no-hex-lint` all exit 0. | `feat(hyg-04): the unreferenced-file and dependency baseline in CI` |
