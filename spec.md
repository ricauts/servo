...
19. **`servo_design_system/` is in `.gitignore` (line 23), but §9 makes it binding.** The loop is told to read `servo_design_system/SKILL.md` and the guideline cards before every UI tick, and every UI item's acceptance requires colours to resolve to `servo_design_system/tokens/*.css`. An ignored directory is not in the repository: a fresh clone, a CI run, or the loop on any other machine has none of it, so those criteria are unverifiable there. The directory is 17 MB, of which 14 MB is `uploads/` (a 7.4 MB `.mp4` and screenshots) — scratch material from the design tool, not the system. The load-bearing part (`SKILL.md`, `readme.md`, `tokens/`, `guidelines/`, `styles.css`) is under 100 KB.
    *Assumption: the loop treats the design system as present-but-uncommitted and does not add it itself — committing 17 MB, including screenshots whose contents the loop has not reviewed, to a public repository is the owner's call, not a tick's. Until the owner decides, UI items keep the token criterion (it is checkable on the owner's machine) and `ds-01` records the gap. The recommended fix is one line: commit the directory and ignore `servo_design_system/uploads/` instead.*
