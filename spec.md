...
| 2026-08-29 | db-05 (owner-directed deploy/claims follow-up) | The deltas `main` genuinely lacked after the db-05 twin collision resolved (the landed implementation stays; PRs #14 and #15 were closed by their own session — see the correction row above): Dockerfile drops the stale `OPS_DATABASE_URL="file:/data/ops.db"` default (the sandbox is Postgres; /api/setup and the ops tools name the variable when it is unset), .env.example activates the sandbox URLs with the passwords scripts/postgres-init.sql actually sets (`servo_ops_rw`/`servo_ops_ro` — the committed example said `servo_ops`, which matches no role), README drops "enforced at the driver" for the role-plus-read-only-transaction wording, ROADMAP and ARCHITECTURE drop the last pre-db-05 sandbox claims, POSITIONING rewrites the compose ledger row and retires the spent `until: db-05` sqlite exemption (docs/CONTRACT.md and docs/ARCHITECTURE.md are sqlite-free, so it guarded nothing). Every content hunk is lifted from closed PR #14 (958978f..1fca950) where it applied verbatim to the landed implementation; tests/claims-audit.test.ts takes that PR's decoupling of three tests from the live canon's db-05 entry, and tests/kb-pdf.test.ts adopts closed PR #15's stronger not-null assertion (30e6186) over the `?? new Uint8Array()` coalesce. | merge `fix/db-05-deploy-claims` (PR #16) |
