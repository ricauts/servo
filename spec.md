...
| 2026-08-28 | reb-07 | `scripts/claims-audit.mjs` makes `docs/POSITIONING.md`'s fenced `banned-phrases` block executable: the script hardcodes no phrase, path or exemption — only the policy file's location and the fence label, both `--policy`-overridable — and adding a phrase to the fence changes enforcement with no code change (proved by adding one in a copy). It expands `scan:` (README, SECURITY, ROADMAP, package.json, `docs/*.md` non-recursive), honours `matching:`, `selfExclude:`, `allow:` and the path-, section- and `maxOccurrences`-scoped `exempt:` entries, and exits 1 with `file:line:col` per hit. `npm run claims:audit` is wired into CI between `lint:hex` and the build. On today's tree: 0 violations, 3 allow-list rescues (README:43 "SaaS endpoint", README:122 and SECURITY:3 "self-hosted"), 22 exempted, 16 of them flagged as pending retirement by `db-01`. Two engine semantics the canon leaves open are decided and documented in the file header: `until:` is metadata, not an expiry (the canon says db-01 *retires the entry*), and `enforced: false` is inert — it grants no exemption, the reading that cannot silently widen the ban. Where the canon's own rules bite, they are followed rather than softened: the hyphen-as-boundary rule that makes "self-hosted" contain "hosted" also makes "multi-cloud version" contain "cloud version", and `allow:` is the declared escape hatch. Adopt-first: **nothing adopted.** Nine candidates were researched in parallel and every licence read from the actual LICENSE file — Vale (MIT, c 2016 Joseph Kato), cspell 10.1.1 (MIT), textlint 15.8.0 + textlint-rule-prh + textlint-filter-rule-comments (MIT), alex 11.0.1 (MIT), retext-profanities/-equality, nlcst-search, markdownlint (MIT). All were rejected on fit: none can source policy from a markdown fence, none has an allow-list that rescues a longer phrase containing a banned one (Vale's `exceptions` sees only the matched substring, forcing the prefix-lookbehind the canon forbids; alex's nlcst-normalize joins "self-hosted" into one token so a `hosted` ban would never fire), none has heading-title section scoping or an occurrence cap, and prh is a substitution engine whose `--fix` would silently rewrite positioning prose. Vale is additionally a glibc-linked Go binary fetched by an unverified platform-branching postinstall — the reproducible-single-image failure that already rejected SheetJS. **Five independent verification rounds ran against the acceptance criteria and the diff, by agents that did not write the code and were told to prove each criterion unmet; twenty-three defects were found and fixed, each now covered by a fixture.** Round 1 caught a BLOCKING false negative (a phrase wrapped across `> ` blockquote lines was invisible — the shape `docs/POSITIONING.md`'s own one-liner and boilerplate are written in) and a policy-parser hole where the valid YAML `banned: ["hosted", ...]` parsed as one nonsense phrase and left the lint green over banned copy. Rounds 2-5 then attacked each fix in turn: the repaired matcher first fused prose across headings, rules, bullets and spaced em dashes (false positives on this repo's actual style), then missed underscore emphasis, inline HTML, link text, code spans, trailing-space and hard-break wraps, non-breaking spaces and HTML entities; the parser silently mis-read a same-indent block sequence, an apostrophe in a value (which swallowed a trailing comment and dropped the phrase), an unquoted colon-bearing phrase (`[object Object]`), a second `banned-phrases` fence, `enforced: no`, a quoted `maxOccurrences`, an empty phrase and an unterminated quote — all now loud failures. The last round's 129,469-mutation reachability sweep (a banned phrase inserted at every offset of every scanned file) found no region of user-visible prose the audit cannot reach, and signed off all four criteria MET. The mask that makes this work is length-preserving, so every reported line and column is the file's own; `README`'s `<img alt="...">` copy is scanned while `href=`/`src=` URLs are not, and each attribute value is bounded so it cannot fuse with the next one or with the element's body. Four CLI tests spawn the script so the exit code and the `file:line:col` line — the form both criterion 1 and criterion 4 are stated in — are covered, not just the exports. KNOWN AND RECORDED, not fixed: `selfExclude.appliesTo: all-scanned-files` is what the canon declares, so a `banned-phrases` fence in any scanned file hides its contents; the engine executes the policy rather than overriding it and reports the hole as a policy warning, and a test asserts this repository has no such fence. Tier A by the classifier and the hint (scripts/ + tests/ + an npm script + a CI step; no dependency, no runtime code): typecheck clean, 412 tests green across 24 files, `claims:audit`, `loop-guard`, `spec-lint` and `lint:hex` all pass. | merge `feat/reb-07` |
