...
import { OPS_SCHEMA_QUERY } from "@/lib/opsdb";
    ...
    if (/password|mfa|locked|2fa/i.test(text)) {
      ...
      if (/create|add|drop|delete|update|insert|alter/i.test(text)) {
        const sql = /drop/i.test(text)
          ...
          : `CREATE TABLE ${slug(ticket.title).replace(/-/g, "_")} (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT NOT NULL, created_at TEXT NOT NULL);`;
        steps.push({
